...
def stream_with_timeout(unit_content, sys_msg):
    ...
    utils.log_token_usage(completion)
...
def evaluate_single():
    ...
    skip_stats = {
        "total_skips": 0,
        "no_question_skips": 0,  # 无问题时跳过
        "question_skips": 0,      # 有问题时跳过
        "skip_details": []        # 详细记录每次跳过
    }
    ...
    with tqdm.tqdm(total=n_units+600, desc="流式输入") as pbar:
        ...
        while current_sec < n_units or qa_idx < len(qa_queue):
            ...
            image_data_uri = utils.pil_to_data_uri(image, fmt="JPEG")
            ...
            if not questions_at_current:
                ...
                success = False
                for attempt in range(args.max_retries + 1):  # 0, 1 → 最多执行2次
                    try:
                        ...
                        if attempt < args.max_retries:
                            print(f"[WARNING] 遇到error: {e}，等待{args.retry_delay}秒后重试（{attempt+1}/{args.max_retries}）")
                            time.sleep(args.retry_delay)
                        else:
                            print(f"[WARNING] 重试{args.max_retries}次后仍失败，跳过")
                            # 记录跳过信息
                            skip_stats["total_skips"] += 1
                            skip_stats["no_question_skips"] += 1
                            skip_stats["skip_details"].append({
                                "type": "no_question",
                                "video_sec": video_sec,
                                "error": str(e),
    ...
    # 保存跳过统计
    skip_stats_path = Path(save_dir) / "skip_statistics.json"
    with open(skip_stats_path, "w", encoding="utf-8") as f:
        json.dump(skip_stats, f, ensure_ascii=False, indent=2)
...
if __name__ == "__main__":
    ...
    save_pred_path = f"{args.base_dir}/outputs/gpt-4o/p{args.p_num}/{timestamp}/eval_predictions_P{args.p_num}.json"
