import types, re, json, math, tqdm, time, os
import torch
import pickle
import tempfile
import numpy as np
...
def stream_with_timeout(unit_content, current_sec):

    pred = ""
    start_time = None 
    last_chunk = None

    response = chat.send_message_stream(unit_content)
    ...
    for chunk in response:
        ...
        if elapsed > args.decision_window:
            break
    ...
    if last_chunk and hasattr(last_chunk, 'usage_metadata'):
        utils.log_token_usage_gemini(last_chunk.usage_metadata, current_sec, log_file=Path(save_dir) / "token_usage.jsonl")
...
def evaluate_single():
    ...
    with tqdm.tqdm(total=n_units+600, desc="流式输入") as pbar:
        ...
        while current_sec < n_units or qa_idx < len(qa_queue):
            ...
            # ========== 3.1 检查当前时间点的问题 ==========
            questions_at_current = []  # 收集所有在 current_sec+1 时刻或之前需要提问的问题
            ...
            video_clip = utils.build_single_unit(video, video_sec, ret_video=True)
            video_bytes = utils.video_clip_to_mp4_bytes(video_clip)
            ...
            if not questions_at_current:
                try:
                    pred, elapsed_time = stream_with_timeout(parts, current_sec)
                    
                except Exception as e:
                    print("[WARNING] 遇到 Gemini ServerError，跳过当前帧")
                    pbar.update(1)
                    continue
            ...
            # ========== 3.3 如果有问题，处理当前时间点的所有问题 ==========
                ...
                for t_end, item, phase, round_id in questions_at_current:
                    ...
                    unit_input = copy.deepcopy(parts)
                    unit_input.append(types.Part(text=question))
                    try:
                        pred, elapsed_time = stream_with_timeout(unit_input, current_sec)
                    # except genai_errors.ServerError as e:
                    except Exception as e:
                        print("[WARNING] 遇到 Gemini ServerError，跳过问题")
                        continue
...
if __name__ == "__main__":
    ...
    save_pred_path = f"{args.base_dir}/outputs/gemini25_pro/p{args.p_num}/{timestamp}/eval_predictions_P{args.p_num}.json"
    ...
    thinking_config = types.ThinkingConfig(
        thinking_budget=128,          # 0 表示 DISABLED  Please choose a value between 128 and 32768.
        include_thoughts=False      # 不包含 thoughts 在响应中
    )

    gen_cfg = types.GenerateContentConfig(
        max_output_tokens=args.max_new_tokens+128,
