...
def stream_with_timeout(unit_content, sys_msg):
    ...
    if args.use_history:
        ...
        if len(history) > args.history_max_len * 2:
            history.pop(0)
        conversations = [sys_msg] + history
    else:
        conversations = [sys_msg] + [unit_content]
    ...
    text = text[0].replace("<|audio_bos|><|AUDIO|><|audio_eos|>", "")
    ...
    text_ids = model.generate(**inputs, use_audio_in_video=True, return_audio=False, 
                                thinker_stopping_criteria=stopping_criteria, 
...
if __name__ == "__main__":
    ...
    save_pred_path = f"{args.base_dir}/outputs/qwen25_omni/p{args.p_num}/{timestamp}/eval_predictions_P{args.p_num}.json"
